...
# 生成证书到该目录
sudo openssl req -x509 -newkey rsa:2048 -keyout /etc/turnserver/turn_server_pkey.pem -out /etc/turnserver/turn_server_cert.pem -days 99999 -nodes

# 修改权限，确保 turnserver 进程能读取
sudo chown turnserver:turnserver /etc/turnserver/turn_server_pkey.pem
sudo chown turnserver:turnserver /etc/turnserver/turn_server_cert.pem
```
...
| `min-port/max-port` | 中继端口范围 | `49152-65535` |
...
在AiRanDesk的 `conf/common.ini` 中配置ICE服务器：
